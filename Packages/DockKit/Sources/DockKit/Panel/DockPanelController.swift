...
@MainActor
public final class DockPanelController: NSObject {
    ...
    let reveal: DockRevealController
    let iconProvider: IconProvider
    let stack: DockStackController
    let appMenuStore: AppMenuStore?
    let minimizedWindowStore: MinimizedWindowStore?
    ...
    var maximumBackingScale: CGFloat = 2
    private var requested: DockSnapshot = .empty
    var snapshot: DockSnapshot = .empty
    var iconTasks: [DockTileID: Task<Void, Never>] = [:]
