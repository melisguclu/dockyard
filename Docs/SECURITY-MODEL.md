...
Four features need the Accessibility API, so it is the only TCC permission Dockyard can hold: tile menus that list a running app's windows, its own commands — *New Window*, *Next Track* — and its recent documents, the minimized-window tiles between the separator and the Trash, the Dock's own tile order and badge counts, and the opt-in *Keep windows clear of the bar*. The terms are deliberately narrow:
...
- **The Dock process is read and never touched.** The Dock's own item list is a walk of one `AXList`: each item's subrole, title, URL, running flag, and `AXStatusLabel`. Nothing is set on the Dock, no action is performed on it, and the two notifications registered on it — item created and item destroyed — carry no payload beyond the element. This is also the reason badges are not instant: the tree posts nothing when a badge changes, and the app will not add a timer to find out.
- **A badge is untrusted text from another application.** `AXStatusLabel` is whatever that app put in its dock tile, so it is treated the way tile labels are: collapsed to a single line, clamped to six characters, dropped when empty, and drawn as text with no interpretation. It is never persisted, and it reaches no code path that can act on it.
...
8. A badge read from the Dock's accessibility tree is display text on the same terms as a label: single line, six characters, dropped when empty. Pinned by `DockItemMatchingTests`.
9. Stage Manager's own preferences are read as booleans with a documented default, so a missing or hand-edited `com.apple.WindowManager` domain changes nothing about where a bar sits.
