...
- On macOS 26 the backdrop is an `NSGlassEffectView` with the regular style, no tint, and the layout's corner radius. The view **is** the bar: its frame is the bar rect, and Liquid Glass draws its own rim, so the app-drawn hairline border is not created on that path. Nothing about the glass is faked with layers; a mask would clip the effect's shape but not the refraction its edges produce, which is most of what distinguishes it from a blur.
- Below that the backdrop is an `NSVisualEffectView` with `.hudWindow`, filling the panel and clipped to the bar by an app-owned mask layer, with the hairline border in a second app-owned layer. It stays clipped rather than resized because AppKit suppresses implicit animations on a view's own backing layer: the view then had to be animated through `NSAnimationContext` while the tiles animated through `CATransaction`, the two interpolated independently, and icons briefly overflowed the bar's ends during the magnification ramp. The glass view does not need the same treatment because nothing animates any more — the display link sets every frame's geometry outright.
